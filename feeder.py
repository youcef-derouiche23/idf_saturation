...
def ingest_validations(spark, logger, config):
    """
    Ingère les validations réseau ferré depuis le fichier CSV
...
def ingest_stations(spark, logger, config):
    ...
    Colonnes : ZdAId, ArRName, ArRTown, ArRFareZone, ArRAccessibility, ArRGeopoint
    """
    logger.info("=" * 60)
    ...
    try:
        ...
        # 1. On crée les colonnes de partitionnement APRÈS avoir chargé le dataframe
        from datetime import datetime
        now = datetime.now()
        ...
        # 2. Sélectionner et renommer les colonnes utiles (inclure les dates de partitionnement !)
        df_stations = df_stations.select(
            F.col("ZdAId").alias("id_station"),  # <-- Clé STIF corrigée pour réparer la carte
            ...
            F.col("year"),   # <-- On les garde pour le partitionnement
            F.col("month"),
            F.col("day")
        )
...
def ingest_regularite(spark, logger, config):
    ...
    try:
        ...
        logger.info(f"Écriture Parquet vers : {hdfs_path}")
        df_regularite.w.write.mode("overwrite").partitionBy("year", "month", "day").parquet(hdfs_path)
